...
class EmailService:
    ...
    def is_configured(
        ...
        When a provider is explicitly supplied, the
        provider-specific configuration is checked.

        This preserves the original LUIP EmailService test
        contract while supporting provider-specific SMTP.
        """
        ...
        try:
            ...
            # -------------------------------------------------
            #
            # Existing LUIP code/tests call:
            #
            #     EmailService.is_configured()
            #
            # and expect the original SMTP_* settings to be
            # evaluated directly.
            #
            # Do NOT resolve this call through
            # SMTP_DEFAULT_PROVIDER.
            # -------------------------------------------------
    ...
    def can_send_live(
        ...
        When provider is omitted, the legacy SMTP configuration
        is checked for backward compatibility.
        """
    ...
    def send_email(
        ...
        This preserves the original LUIP EmailService
        interface and allows existing monkeypatched tests
        to continue working.

        The selected provider is still resolved by the
        ...
        #
        # Existing tests intentionally monkeypatch:
        #
        #     lambda: True
        #
        # or:
        ...
        #     lambda: False
        #
        # Therefore the service-level compatibility contract
        # requires a zero-argument call.
        # -------------------------------------------------
